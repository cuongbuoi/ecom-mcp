...
const saveReviewsToolSchema = z.object({
  ...
  platform: z
    .enum(['aliexpress', 'amazon', 'temu', 'ebay', 'etsy'])
    .describe('The platform name (aliexpress, amazon, temu, ebay, etsy)'),
  reviews: z
    .array(
      z.object({
        review_id: z
          .string()
          .describe(
            'The review ID. If platform is temu, the review_id will be generated using the author, content and created_at with md5 hash.'
          ),
    ...
    .describe(
      'The reviews to save. If platform is temu, the review_id will be generated using the author, content and created_at with md5 hash.'
    )
})
    ...
    try {
      ...
      if (platform === 'temu') {
        args.reviews.forEach((review) => {
          review.review_id = createHash('md5')
            .update(`${review.author}-${review.content}-${review.created_at}`)
            .digest('hex')
        })
      }
