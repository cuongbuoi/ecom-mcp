...
function fixImports(filePath) {
  ...
  // Function to get the relative path from current file to target file
  function getRelativePath(fromFile, toPath) {
    const fromDir = path.dirname(fromFile)
    const relPath = path.relative(fromDir, toPath).replace(/\\/g, '/')
  ...
  while ((match = aliasImportRegex.exec(content)) !== null) {
    ...
    for (const [alias, targetPath] of Object.entries(aliasMap)) {
      if (importPath.startsWith(alias)) {
        ...
        const actualPath = importPath.replace(alias, targetPath)
